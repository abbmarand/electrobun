...
async function buildNative() {
	if (OS === "macos") {
		...
		const wgpuIncludeFlag = existsSync(wgpuIncludeDir)
			? `-I${wgpuIncludeDir}`
			: "";
		const macNativeMinVersion = "-mmacosx-version-min=14.0";
		await $`mkdir -p src/native/macos/build && xcrun --sdk macosx clang++ -c src/native/macos/nativeWrapper.mm -o src/native/macos/build/nativeWrapper.o -fobjc-arc -fno-objc-msgsend-selector-stubs -I./vendors/cef ${wgpuIncludeFlag} -std=c++20 ${macNativeMinVersion}`;
		await $`mkdir -p src/native/build && xcrun --sdk macosx clang++ -o src/native/build/libNativeWrapper.dylib src/native/macos/build/nativeWrapper.o ./vendors/zig-asar/libasar.dylib ${macNativeMinVersion} -framework Cocoa -framework WebKit -framework QuartzCore -framework Metal -framework MetalKit -framework AVFoundation -framework Contacts -framework EventKit -framework Photos -framework CoreLocation -framework CoreBluetooth -framework Speech -framework IOKit -framework UserNotifications -framework Vision -framework CoreImage -framework UniformTypeIdentifiers -F./vendors/cef/Release -weak_framework 'Chromium Embedded Framework' -L./vendors/cef/build/libcef_dll_wrapper -lcef_dll_wrapper -stdlib=libc++ -shared -install_name @executable_path/libNativeWrapper.dylib -Wl,-rpath,@executable_path`;
	} else if (OS === "win") {
