...
function getWindowsIconPath(
	...
	const iconSourcePath =
		icon.startsWith("/") || icon.match(/^[a-zA-Z]:/)
			? icon
			: join(projectRoot, icon);
	...
	console.warn(
		`Warning: NSIS installer icon must be an .ico file. Skipping ${iconSourcePath}`,
	);
	return "";
}
